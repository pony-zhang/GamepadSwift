...
public class InputMapper {
    ...
    private func setupAnalogTimer() {
        // 创建一个定时器来平滑处理模拟输入
        analogTimer = Timer.scheduledTimer(withTimeInterval: 1.0/60.0, repeats: true) { [weak self] _ in
    ...
    private func updateAnalogInputs() {
        ...
        for (stickId, values) in lastStickValues {
            ...
            // 应用灵敏度
            let dx = values.x * stickMapping.sensitivity * config.settings.mouseSpeed
            let dy = values.y * stickMapping.sensitivity * config.settings.mouseSpeed
